...
object DigestDifferencingUtils {
  ...
  def differenceAggregates(ds1:Seq[ScanResultEntry],
                           ...
                           constraints:Seq[ScanConstraint]) : Seq[QueryAction] = {
    ...
    val namedConstraints = indexConstraintsByAttrName(constraints)

    def evaluate(partitions:Map[String, String]) = try {
        val empty = ds1.isEmpty || ds2.isEmpty

        // The next set of constraints is all the constraints we had that weren't also bucketed, plus any new
        // constraints generated by the category functions
        val nonBucketedConstraints = constraints.filter(c => !namedBuckets.containsKey(c.getAttributeName))
        val bucketConstraints = bucketing.map(f => f.constrain(namedConstraints.get(f.getAttributeName), partitions(f.getAttributeName)))
        val nextConstraints = nonBucketedConstraints ++ bucketConstraints
  ...
  private def indexConstraintsByAttrName(constraints:Seq[ScanConstraint]) =
    constraints.map(c => c.getAttributeName -> c).toMap
